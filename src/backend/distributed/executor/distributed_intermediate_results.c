								 ...
								 bool errorOnAnyFailure)
{
	...
	/*
	 * Local execution is not supported because here we use task->perPlacementQueryStrings.
	 * Local execution does not know how to handle it. One solution is to extract and set
	 * task->queryStringLazy from perPlacementQueryStrings. The extracted one should be the
